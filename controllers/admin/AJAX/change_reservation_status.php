...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    $reservation_id = filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT);
    $status = $_POST['status'];
    ...
    if (!empty($reservation_id)) {
        ...
        $updateResult = Reservation::update_reservation_status($reservation_id, $status,$changeBy);

        if($updateResult['success']){
            $response = array('success' => true, 'message' => 'Reservation status updated successfully.');
        }else{
            $response = array('success' => false, 'message' => 'Reservation status could not be updated.');
        }
        ...
        $response = array('success' => false, 'error' => 'Reservation ID is required.');
        header('Content-Type: application/json');
        echo json_encode($response);
    }
} else {
